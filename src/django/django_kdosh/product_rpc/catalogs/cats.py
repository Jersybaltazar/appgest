...
from ..models import ProductCategory, PosCategory, ProductAttribute, ProductAttributeValue, ResPartner
# from models import
...
def update_product_catalogs():
    ...
    # POS CATEGORY
    cat_pos_list = models.execute_kw(db, uid, password,
    ...
    # PRODUCT ATTRIBUTE
    attr_list = models.execute_kw(db, uid, password,
    ...
    attr_line_list = models.execute_kw(db, uid, password,
    ...
    for item in cat_pos_list:
        cat_pos_objs.append(PosCategory(
            id=item["id"],
            parent_id=0 if type(item["parent_id"]) == bool else item["parent_id"][0],
            name=item["name"],
            display_name=item["display_name"],
    ...
    for item in attr_list:
        attr_objs.append(ProductAttribute(
            id=item["id"],
            name=item["name"]
        ))
    ...
    for item in attr_line_list:
        attr_line_objs.append(ProductAttributeValue(
            id=item["id"],
...
def get_product_catalogs():
    ...
    catalogs["pos_category"] = [tuple_to_dictionary(pos, pos_mapper) for pos in pos_category]
    ...
    catalogs["product_attribute"] = [tuple_to_dictionary(attr, attribute_mapper) for attr in attribute]
    ...
    catalogs["product_attribute_value"] = [tuple_to_dictionary(attr_val, attribute_val_mapper) for attr_val in attribute_val]
...
def update_order_catalogs():
    ...
    for item in res_partner_list:
        res_partner_objs.append(ResPartner(
            id=item["id"],
            name=item["name"],
            vat=item["vat"]
        ))
...
def get_order_catalogs():
    ...
    catalogs["res_partner"] = [tuple_to_dictionary(partner, partner_mapper) for partner in res_partner]
